...
def create_user():
    ...
    if request.method == "POST": 
        ...
        form_infa['first_name'] = first_name
        second_name = request.form.get('lastname')
        form_infa['second_name'] = second_name
        middle_name = request.form.get('middlename')
        form_infa['middle_name'] = middle_name
        login = request.form.get('login')
        form_infa['login'] = login
        password = request.form.get('password')
        ...
        if errors:
            roles = get_roles()
            return render_template('create_user.html', roles=roles, errors=errors, form_infa=form_infa)
        ...
        try:
            ...
            connection.rollback()
            flash(f'Ошибка при регистрации: {err}', 'danger')
            app.logger.error(f'Ошибка при регистрации: {err}')
        finally:
            cursor.close()
            connection.close()
